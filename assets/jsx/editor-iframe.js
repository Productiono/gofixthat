    ...
    cscoEditorIframe = {
      ...
      */
        ...
        setInterval(function () {
          ...
          if (currentValue !== $this.prevStorageVal) {
            ...
            if (currentValue) {
              ...
              if (data.hasOwnProperty('pageAddedTemplate')) {
                $this.pageAddedTemplate = data.pageAddedTemplate;
                $this.setPageAddedTemplate();
              }
            }
          }
        }, 100);
      ...
      rootObserver: function rootObserver() {
        var update = false;
        if (!$this.html.classList.contains('mbf-editor-iframe')) {
          $this.html.classList.add('mbf-editor-iframe');
          update = true;
        }
        if (!$this.body.classList.contains('mbf-editor-styles-wrapper')) {
          $this.body.classList.add('mbf-editor-styles-wrapper');
          update = true;
        }
        if (update) {
          $this.setBreakpoints();
      ...
      */
      ...
      setLayout: function setLayout() {
        $this.body.classList.remove('mbf-sidebar-enabled');
        $this.body.classList.remove('mbf-sidebar-disabled');
        $this.body.classList.add($this.page_layout);
        $this.body.classList.add($this.post_type);
      },
      /*
       * Set Page Added Template
       */
      setPageAddedTemplate: function setPageAddedTemplate() {
        if (null === $this.pageAddedTemplate) return;
        var settings = $this.pageAddedTemplate;
        if (settings.hasOwnProperty('addedTemplate') && settings.addedTemplate) {
          settings.addedTemplate.forEach(function (currentValue, index, array) {
            $this.body.classList.remove(array[index]);
          });
        }
        if (settings.hasOwnProperty('currentTemplate') && settings.currentTemplate) {
          $this.body.classList.add(settings.currentTemplate);
        }
      }
    };
